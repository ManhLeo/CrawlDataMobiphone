...
def get_brand_from_name(name):
    """Lấy brand từ từ đầu tiên của tên điện thoại"""
    if name:
        return name.split()[0]
    return None
...
def get_price(source):
    ...
    # Tìm trong class item cf-left
    item_div = source.find('div', class_='item cf-left')
    if item_div:
        # Chỉ lấy giá từ thẻ b
